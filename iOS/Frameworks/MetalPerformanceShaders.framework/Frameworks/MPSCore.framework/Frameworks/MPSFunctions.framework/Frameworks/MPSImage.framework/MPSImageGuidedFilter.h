//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <MPSCore/MPSKernel.h>

@class MPSImageBilinearScale, MPSImageBox, MPSImageLanczosScale;

@interface MPSImageGuidedFilter : MPSKernel
{
    unsigned long long _kernelDiameter;
    float _epsilon;
    float _reconstructScale;
    float _reconstructOffset;
    _Bool _rescaleCoefficients;
    _Bool _reconstructionCoefficientsSampleBicubic;
    MPSImageBox *_boxFilter;
    MPSImageLanczosScale *_lanczosScaler;
    MPSImageBilinearScale *_bilinearScaler;
}

+ (const struct MPSLibraryInfo *)libraryInfo:(void *)arg1;
@property(nonatomic) _Bool reconstructionCoefficientsSampleBicubic; // @synthesize reconstructionCoefficientsSampleBicubic=_reconstructionCoefficientsSampleBicubic;
@property(nonatomic) _Bool rescaleCoefficients; // @synthesize rescaleCoefficients=_rescaleCoefficients;
@property(nonatomic) float reconstructOffset; // @synthesize reconstructOffset=_reconstructOffset;
@property(nonatomic) float reconstructScale; // @synthesize reconstructScale=_reconstructScale;
@property(nonatomic) float epsilon; // @synthesize epsilon=_epsilon;
@property(readonly, nonatomic) unsigned long long kernelDiameter; // @synthesize kernelDiameter=_kernelDiameter;
- (void)encodeReconstructionToCommandBuffer:(id)arg1 guidanceTexture:(id)arg2 coefficientsTextureA:(id)arg3 coefficientsTextureB:(id)arg4 destinationTexture:(id)arg5;
- (void)encodeReconstructionToCommandEncoder:(id)arg1 commandBuffer:(id)arg2 guidanceTexture:(id)arg3 coefficientsTextureA:(id)arg4 coefficientsTextureB:(id)arg5 destinationTexture:(id)arg6;
- (void)encodeReconstructionInternalToCommandEncoder:(id)arg1 commandBuffer:(id)arg2 guidanceTexture:(id)arg3 coefficientsTextureA:(id)arg4 coefficientsTextureB:(id)arg5 destinationTexture:(id)arg6;
- (void)encodeRegressionToCommandBuffer:(id)arg1 sourceTexture:(id)arg2 guidanceTexture:(id)arg3 weightsTexture:(id)arg4 destinationCoefficientsTextureA:(id)arg5 destinationCoefficientsTextureB:(id)arg6;
- (void)encodeRegressionToCommandEncoder:(id)arg1 commandBuffer:(id)arg2 sourceTexture:(id)arg3 guidanceTexture:(id)arg4 weightsTexture:(id)arg5 destinationCoefficientsTextureA:(id)arg6 destinationCoefficientsTextureB:(id)arg7;
- (void)encodeRegressionInternalToCommandEncoder:(id)arg1 commandBuffer:(id)arg2 sourceTexture:(id)arg3 guidanceTexture:(id)arg4 weightsTexture:(id)arg5 destinationCoefficientsTextureA:(id)arg6 destinationCoefficientsTextureB:(id)arg7;
- (void)encodeReconstructionToCommandBuffer:(id)arg1 guidanceTexture:(id)arg2 coefficientsTexture:(id)arg3 destinationTexture:(id)arg4;
- (void)encodeReconstructionToCommandEncoder:(id)arg1 commandBuffer:(id)arg2 guidanceTexture:(id)arg3 coefficientsTexture:(id)arg4 destinationTexture:(id)arg5;
- (void)encodeReconstructionInternalToCommandEncoder:(id)arg1 commandBuffer:(id)arg2 guidanceTexture:(id)arg3 coefficientsTexture:(id)arg4 destinationTexture:(id)arg5;
- (void)encodeRegressionToCommandBuffer:(id)arg1 sourceTexture:(id)arg2 guidanceTexture:(id)arg3 weightsTexture:(id)arg4 destinationCoefficientsTexture:(id)arg5;
- (void)encodeRegressionToCommandEncoder:(id)arg1 commandBuffer:(id)arg2 sourceTexture:(id)arg3 guidanceTexture:(id)arg4 weightsTexture:(id)arg5 destinationCoefficientsTexture:(id)arg6;
- (void)encodeRegressionInternalToCommandEncoder:(id)arg1 commandBuffer:(id)arg2 sourceTexture:(id)arg3 guidanceTexture:(id)arg4 weightsTexture:(id)arg5 destinationCoefficientsTexture:(id)arg6;
- (void)dealloc;
- (id)copyWithZone:(struct _NSZone *)arg1 device:(id)arg2;
- (void)encodeWithCoder:(id)arg1;
- (id)initWithCoder:(id)arg1 device:(id)arg2;
- (id)initWithDevice:(id)arg1 kernelDiameter:(unsigned long long)arg2;
- (void)initFilterInfoWithDevice:(id)arg1;

@end

