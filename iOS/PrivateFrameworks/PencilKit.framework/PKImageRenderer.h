//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

@class PKMetalRendererController;

@interface PKImageRenderer : NSObject
{
    _Bool _synchronous;
    double _scale;
    PKMetalRendererController *_rendererController;
    long long _cancelCount;
    struct CGSize _size;
}

+ (void)_fuzzTestUnzippedData:(id)arg1;
- (void).cxx_destruct;
@property(nonatomic) _Bool synchronous; // @synthesize synchronous=_synchronous;
@property(nonatomic) long long cancelCount; // @synthesize cancelCount=_cancelCount;
@property(retain, nonatomic) PKMetalRendererController *rendererController; // @synthesize rendererController=_rendererController;
@property(readonly, nonatomic) double scale; // @synthesize scale=_scale;
@property(readonly, nonatomic) struct CGSize size; // @synthesize size=_size;
@property(readonly, nonatomic) _Bool sixChannelBlending;
@property(nonatomic) _Bool invertColors;
- (void)buildStrokeRenderCacheForDrawing:(id)arg1;
- (void)resume;
- (void)cancel;
- (void)sixChannelCGRenderStrokes:(id)arg1 clippedToStrokeSpaceRect:(struct CGRect)arg2 scale:(double)arg3 completion:(CDUnknownBlockType)arg4;
- (void)sixChannelCGRenderDrawing:(id)arg1 clippedToStrokeSpaceRect:(struct CGRect)arg2 scale:(double)arg3 completion:(CDUnknownBlockType)arg4;
- (void)renderStrokes:(id)arg1 clippedToStrokeSpaceRect:(struct CGRect)arg2 scale:(double)arg3 completion:(CDUnknownBlockType)arg4;
- (void)_renderCGStrokes:(id)arg1 clippedToStrokeSpaceRect:(struct CGRect)arg2 scale:(double)arg3 completion:(CDUnknownBlockType)arg4;
- (void)renderCGStrokes:(id)arg1 clippedToStrokeSpaceRect:(struct CGRect)arg2 scale:(double)arg3 completion:(CDUnknownBlockType)arg4;
- (void)renderDrawing:(id)arg1 clippedToStrokeSpaceRect:(struct CGRect)arg2 scale:(double)arg3 completion:(CDUnknownBlockType)arg4;
- (void)renderDrawing:(id)arg1 completion:(CDUnknownBlockType)arg2;
- (void)setupWithSize:(struct CGSize)arg1 scale:(double)arg2 renderQueue:(id)arg3 sixChannelBlending:(_Bool)arg4 synchronous:(_Bool)arg5;
- (void)dealloc;
- (id)initSyncWithSize:(struct CGSize)arg1 scale:(double)arg2;
- (id)initWithSize:(struct CGSize)arg1 scale:(double)arg2 renderQueue:(id)arg3 sixChannelBlending:(_Bool)arg4;
- (id)initWithSize:(struct CGSize)arg1 scale:(double)arg2 renderQueue:(id)arg3;
- (id)initWithSize:(struct CGSize)arg1 scale:(double)arg2 useMetal:(_Bool)arg3 renderQueue:(id)arg4;
- (id)initWithSize:(struct CGSize)arg1 scale:(double)arg2 useMetal:(_Bool)arg3;
- (id)initWithSize:(struct CGSize)arg1 scale:(double)arg2;

@end

