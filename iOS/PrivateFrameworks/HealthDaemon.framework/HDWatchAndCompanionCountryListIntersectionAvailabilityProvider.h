//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

#import <HealthDaemon/HDObservableRegionAvailabilityProviding-Protocol.h>
#import <HealthDaemon/HDRegionAvailabilityDataSourceObserver-Protocol.h>

@class NSString, NSUUID;
@protocol HDPairedDeviceCapabilityProviding, HDRegionAvailabilityDataSource, HDRegionAvailabilityProvidingDelegate, OS_os_log;

@interface HDWatchAndCompanionCountryListIntersectionAvailabilityProvider : NSObject <HDRegionAvailabilityDataSourceObserver, HDObservableRegionAvailabilityProviding>
{
    id <HDRegionAvailabilityDataSource> _regionAvailabilityDataSource;
    id <HDPairedDeviceCapabilityProviding> _pairedDeviceCapabilityProvider;
    NSUUID *_featureCapability;
    NSObject<OS_os_log> *_loggingCategory;
    id <HDRegionAvailabilityProvidingDelegate> _delegate;
}

- (void).cxx_destruct;
- (void)regionAvailabilityDataSourceDidUpdateActiveRemoteCountrySet:(id)arg1;
- (void)setDelegate:(id)arg1 queue:(id)arg2;
- (id)regionAvailabilityForDevice:(id)arg1;
- (id)regionAvailability;
- (id)onboardingEligibilityForCountryCode:(id)arg1 device:(id)arg2;
- (id)onboardingEligibilityForCountryCode:(id)arg1;
- (_Bool)isCountryCodeAllowed:(id)arg1;
- (id)initWithRegionAvailabilityDataSource:(id)arg1 pairedDeviceCapabilityProvider:(id)arg2 featureCapability:(id)arg3 loggingCategory:(id)arg4;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

