//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

@class NSNotification, PDFPage, PPKPDFThumbnailViewBase;

@protocol PPKPDFThumbnailCollectionViewInterface
- (void)reloadPageAt:(long long)arg1;
- (void)reloadPage:(PDFPage *)arg1;
- (void)pageChanged:(NSNotification *)arg1;
- (void)currentPageChanged:(NSNotification *)arg1;
- (void)documentMutated:(NSNotification *)arg1;
- (void)documentChanged:(NSNotification *)arg1;
- (PDFPage *)currentPage;
- (void)updateIconsImages;
- (id)initFromThumbnailView:(PPKPDFThumbnailViewBase *)arg1;
@end

