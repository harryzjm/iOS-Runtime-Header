//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

@class FTTextToSpeechSpeechFeatureInputPhonemeSequence, NSData, NSMutableDictionary, NSString;

__attribute__((visibility("hidden")))
@interface FTTextToSpeechSpeechFeatureRequest_LexiconEntry : NSObject
{
    NSMutableDictionary *_storage;
    NSData *_data;
    const struct LexiconEntry *_root;
}

- (void).cxx_destruct;
- (id)flatbuffData;
- (Offset_dc6a516e)addObjectToBuffer:(void *)arg1;
@property(readonly, nonatomic) FTTextToSpeechSpeechFeatureInputPhonemeSequence *value;
@property(readonly, nonatomic) NSString *key;
- (id)copyWithZone:(struct _NSZone *)arg1;
- (id)initWithFlatbuffData:(id)arg1 root:(const struct LexiconEntry *)arg2 verify:(_Bool)arg3;
- (id)initWithFlatbuffData:(id)arg1 root:(const struct LexiconEntry *)arg2;
- (id)initAndVerifyWithFlatbuffData:(id)arg1;
- (id)initWithFlatbuffData:(id)arg1;

@end

