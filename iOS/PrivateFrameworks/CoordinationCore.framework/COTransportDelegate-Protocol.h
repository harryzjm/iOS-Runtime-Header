//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <CoordinationCore/NSObject-Protocol.h>

@class COConstituent, COMeshCommand, COMeshRequest, COOnDemandNodeCreationRequest, NSError;
@protocol COTransportProtocol;

@protocol COTransportDelegate <NSObject>

@optional
- (void)transport:(id <COTransportProtocol>)arg1 didUpdateRemoteConstituent:(COConstituent *)arg2 to:(COConstituent *)arg3;
- (void)transport:(id <COTransportProtocol>)arg1 shouldUpdateRemoteConstituent:(COConstituent *)arg2 to:(COConstituent *)arg3 forCommand:(COMeshCommand *)arg4 completionHandler:(void (^)(_Bool))arg5;
- (void)transport:(id <COTransportProtocol>)arg1 willUpdateRemoteConstituent:(COConstituent *)arg2 to:(COConstituent *)arg3;
- (void)transport:(id <COTransportProtocol>)arg1 didInvalidateWithError:(NSError *)arg2;
- (void)transport:(id <COTransportProtocol>)arg1 didReceiveError:(NSError *)arg2 forCommand:(COMeshCommand *)arg3;
- (void)transport:(id <COTransportProtocol>)arg1 didReceiveOnDemandNodeCreationRequest:(COOnDemandNodeCreationRequest *)arg2;
- (void)transport:(id <COTransportProtocol>)arg1 didReceiveRequest:(COMeshRequest *)arg2 callback:(void (^)(COMeshResponse *, NSError *))arg3;
- (void)transport:(id <COTransportProtocol>)arg1 didReceiveCommand:(COMeshCommand *)arg2;
@end

