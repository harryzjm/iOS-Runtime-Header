//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <DocumentManagerExecutables/DOCUIAppearanceValues-Protocol.h>

@class DOCTag, NSString, UIColor, UIImageSymbolConfiguration;
@protocol DOCTagContainerAppearanceProviding;

@protocol DOCTagPickerCellAppearanceProviding <DOCUIAppearanceValues>
@property(readonly) double spacing_titleToCheckmark;
@property(readonly) double spacing_trailingEdgeToCheckmark;
@property(readonly) double spacing_dotToTitle;
@property(readonly) double spacing_leadingEdgeToDot;
@property(readonly) double cellCornerRadius;
@property(readonly) double dotSize;
@property(readonly) struct NSDirectionalEdgeInsets cellHoverEffectInsetsForEditingTextField;
@property(readonly) struct NSDirectionalEdgeInsets cellHoverEffectInsets;
@property(readonly) struct NSDirectionalEdgeInsets cellInteriorLayoutMargins;
@property(readonly) struct NSDirectionalEdgeInsets cellExternalMargins;
@property(readonly) NSString *titleTextStyle;
@property(readonly) UIImageSymbolConfiguration *checkmarkImageConfiguration;
@property(readonly) _Bool addTagCellShowsDashedBorder;
@property(readonly) id <DOCTagContainerAppearanceProviding> container;
- (UIColor *)backgroundColorForTag:(DOCTag *)arg1 selected:(_Bool)arg2;
- (UIColor *)checkmarkImageColorForTag:(DOCTag *)arg1;
@end

